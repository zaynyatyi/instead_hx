...
revolver = obj
{
    ...
    exam = function (s)
        ...
        В барабане должно было быть шесть патронов. Но сейчас
...
hotel_bed = obj
{
    ...
    useit = function (s)
        p [[Ну уж нет! Я даже представлять не хочу как использовали эту
        кровать до меня. Не один раз. И я не хочу больше здесь не только
        лежать, но и оставаться в этой комнате вообще.
        ]];
        return;
    end,
    used = function (s, w)
    ...
    talk = function (s, w)
        ...
        неразделённой любви. Но я был не в той ситуации. Да и не привык я
        жаловаться о своих проблемах неодушевлённым предметам.
        ]];
        return;
    end,
};
...
hotel_bedside_table = obj
{
    ...
    dsc = nil,
    exam = function (s)
        p 'Я осмотрел тумбочку. Она была пуста.';
        return;
    end,
    useit = function (s)
...
hotel_window_5 = obj
{
    ...
    dsc = nil,
    exam = function (s)
        p 'Это то окно, из которого я выпрыгнул.';
        return;
    end,
    take = function (s)
        p [[Окно невозможно взять с собой.
...
hotel_fire_stairs_floor_1z = room
{
    ...
    pic = 'images/hotel_fire_stairs_floor_1.png',
    dsc = function (s)
        p [[Я, шатаясь, попытался выбраться из кучи пакетов. Но споткнулся, и,
        потеряв равновесие, упал, ударившись головой о мусорный бак. В глазах
        вспыхнули звёзды, а потом наступила тьма...
        ]];
        return;
    end,
...
police_car_8 = room
{
    ...
    dsc = function (s)
        ...
        у него имплант от «Сайямура медикалз» вместо лёгкого.^
...
felchi_key = obj
{
    ...
    dsc = nil,
    exam = function (s)
        p 'Ключ на брелоке с небольшой обезьянкой.';
        if not s._examined then
            s._examined = true;
            p [[Ключ соответствовал современным тенденциям в повышении степени
            сохранности своих домов.
            ]];
        end
        p [[Он был одновременно и механическим и содержал электронику. Судя
        по всему, это ключ от дома угрюмого типа.
        ]];
        return;
    end,
    take = function (s)
...
quay_red_car = obj
{
    ...
    useit = function (s)
        if not subway._felchi_street then
            p 'Я не знал куда бы мне отправиться.';
            return;
        else
...
flashlight = obj
{
    ...
    exam = function (s)
        if s._batteries then
            p [[Металлический фонарик. Батарейка вроде бы рабочая. Однако,
            лампочка в фонарике была разбита, поэтому я не знал исправен ли
            он.
            ]];
        else
...
part_01_end = room
{
    ...
    dsc = function (s)
        ...
        выбора.^
        Я успел вовремя. Навстречу мне попались несколько полицейских
